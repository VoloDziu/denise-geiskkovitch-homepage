    ...
    <li>
      <Link
        ...
        className={clsx(
          ...
          isActive
            ? 'text-teal-500 dark:text-purple-400'
            : 'hover:text-teal-500 dark:hover:text-purple-400',
        )}
      >
        {children}
        {isActive && (
          <span className="absolute inset-x-1 -bottom-px h-px bg-linear-to-r from-teal-500/0 via-teal-500/40 to-teal-500/0 dark:from-purple-400/0 dark:via-purple-400/40 dark:to-purple-400/0" />
        )}
      </Link>
    </li>
  )
...
function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <MoonIcon className="hidden h-6 w-6 fill-zinc-700 stroke-zinc-500 transition dark:block [@media_not_(prefers-color-scheme:dark)]:fill-purple-400/10 [@media_not_(prefers-color-scheme:dark)]:stroke-teal-500 [@media(prefers-color-scheme:dark)]:group-hover:stroke-zinc-400" />
    </button>
  )
}
